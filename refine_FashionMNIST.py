...
from models import HD_encoder, LeHDC, BPRFHD
...
def main():
    ...
    # get mask for dimension reduction
    mask_dim = torch.load('save_FashionMNIST/mask_dim.pth')
    mask_dim = mask_dim[0:args.HV_dim]

    if len(args.multigpu) > 1:
        model = nn.DataParallel(BPRFHD(hd_full_dim=args.HV_dim, n_classes=n_classes,
                                       mask_dim=mask_dim, full_dim_am=init_am, dropout=args.dropout),
                                device_ids=device)
    else:
        model = BPRFHD(hd_full_dim=args.HV_dim, n_classes=n_classes,
                       mask_dim=mask_dim, full_dim_am=init_am, dropout=args.dropout)
    ...
    for epoch in range(num_epochs):
        ...
        if isinstance(model, nn.DataParallel):
            torch.save(model.module.state_dict(), f'refine_model_epoch_{epoch + 1}.pth')
        else:
            torch.save(model.state_dict(), f'refine_model_epoch_{epoch + 1}.pth')
