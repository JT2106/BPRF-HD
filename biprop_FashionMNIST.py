...
from models import HD_encoder, LeHDC, BipropHD
...
def train(model, train_loader, optimizer, device, hd_rand_proj, args):
    ...
    for batch in train_loader:
        ...
        outputs, _, _, _ = model(HV_feature)
...
def validate(model, val_loader, device, hd_rand_proj, args):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            outputs, query_vectors, associate_memory, mask_dim = model(HV_feature)
            loss = nn.CrossEntropyLoss()(outputs, label)
            ...
            similarity_metrics = hd.dot_similarity(query_vectors, associate_memory)
    ...
    return epoch_loss, epoch_acc, epoch_acc_hd, mask_dim
...
def main():
    ...
    # get weights for the HD mapper
    hd_rand_proj = torch.load('save_FashionMNIST/hd_rand_proj.pth')

    # get weights for the HD classifier
    pretrained_model = LeHDC(n_dimensions=args.HV_dim, n_classes=n_classes, dropout=args.dropout)
    pretrained_model.load_state_dict(torch.load('save_FashionMNIST/pretrain_model.pth'))
    for param in pretrained_model.parameters():
        init_am = param.sign()

    if len(args.multigpu) > 1:
        model = nn.DataParallel(BipropHD(n_dimensions=args.HV_dim, n_classes=n_classes,
                                         init_am=init_am, dim_reduction=args.dim_rdc, dropout=args.dropout),
                                device_ids=device)
    else:
        model = BipropHD(n_dimensions=args.HV_dim, n_classes=n_classes,
                         init_am=init_am, dim_reduction=args.dim_rdc, dropout=args.dropout)
    ...
    for epoch in range(num_epochs):
        ...
        # val_loss, val_acc, _, _ = validate(model, val_loader, device, hd_rand_proj, args)

        scheduler.step(train_loss)

        # 保存每个模型的维度掩码
        mask_dim = mask_dim[0:args.HV_dim]
        torch.save(mask_dim, f'mask_dim_{epoch + 1}.pth')
